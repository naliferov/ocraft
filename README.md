...
- a **node API server** backing a **Vue 3 + p5.js visual editor**,
- and standalone **MCP servers** (Telegram, Google Calendar) that let an AI assistant read from your accounts.
...
Each entry receives a `ctx`: `args`, `env`, `log(msg)`, persistent `state.load()/save()`, and `time.now()`. Existing entries include `telegram-reminder`, `telegram-poll`, `img-optimize[-dir]`, `deploy`, `every-hour`, and `test`.
