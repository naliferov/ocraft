...
- **Visual editor** (`frontend/`) — *one* client: a Vue 3 tree browser plus a per-type editor/preview (a code editor for scripts, rich text for html). The app is literally a node router — just `/` and `/node/:id`.
...
ocraft/
  ...
  runtime/          # API server (apiServer.js) + CLI entrypoint (cli.js) + automation engine — runners (AI), task executor/scheduler, service supervisor, tasks/, services/, state/
...
```bash
node runtime/cli.js service start api   # node API server (runtime/apiServer.js) on :3001
