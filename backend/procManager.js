...
const listConfigs = async () => {
  let files
  try { files = await fsp.readdir(PROCS_DIR) } catch { return [] }
...
const loadProcState = async (id) => {
  try { return JSON.parse(await fsp.readFile(statePath(id), 'utf-8')) } catch { return {} }
}
...
const markFailed = (id, message) => withLock(lockName(id), async () => {
  const rec = await loadProcState(id)
  await saveProcState(id, { ...rec, status: 'failed', pid: null, error: message, stoppedAt: getTime() })
})
...
export const startProc = (id) => withLock(lockName(id), async () => {
  const cfg = await loadConfig(id)
  if (!cfg) throw new Error(`No proc config "${id}"`)
  ...
  if (!child.pid) { // synchronous spawn failure
  ...
  child.unref()
  const started = { pid: child.pid, status: 'running', startedAt: getTime(), stoppedAt: null, error: null }
  await saveProcState(id, started)
  return statusOf(cfg, started)
})

export const stopProc = (id, { graceMs = 3000 } = {}) => withLock(lockName(id), async () => {
  const cfg = await loadConfig(id)
  if (!cfg) throw new Error(`No proc config "${id}"`)
