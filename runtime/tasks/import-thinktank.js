...
// nodes are never touched). Run: `node bin/cli.js run import-thinktank [vaultPath]`.
...
const REPO_ROOT = path.join(here, '../..') // runtime/tasks → repo root
const STORE_DIR = path.join(REPO_ROOT, 'kernel/data') // the node store lives in the kernel
const NODES_DIR = path.join(STORE_DIR, 'nodes')
const ASSETS_IMG_DIR = path.join(STORE_DIR, 'assets/img')
const ASSETS_OPT_DIR = path.join(ASSETS_IMG_DIR, 'optimized')
const MANIFEST_PATH = path.join(STORE_DIR, 'thinktank-import.json')
const DEFAULT_VAULT = path.join(REPO_ROOT, '../ThinkTank') // sibling of the repo
