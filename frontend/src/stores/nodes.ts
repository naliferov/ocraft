...
import type { NodeData, TreeNode } from '../types'
...
export const useNodesStore = defineStore('nodes', () => {
  ...
  const deletedNodes = ref<NodeData[]>([])
  ...
  const tree = computed<TreeNode[]>(() => {
    const byId = new Map<string, TreeNode>(
      nodes.value.map((node): [string, TreeNode] => [node.id, { ...node, children: [] }]),
    )
    ...
    const byName = (left: TreeNode, right: TreeNode) =>
  ...
  const pathOf = (id) => {
    const byId = new Map<string, NodeData>(
      nodes.value.map((node): [string, NodeData] => [node.id, node]),
    )
    const parts: string[] = []
    const seen = new Set()
    let cur = byId.get(id)
