...
// On entering edit mode, start in rich mode and seed the contenteditable from the
// stored HTML. We set innerHTML imperatively (not v-html) so Vue never re-renders
// it out from under the cursor while typing.
...
<template>
  <div class="html-node">
    <div class="bar">
      ...
      <n-button v-else size="small" @click="editing = true">Edit</n-button>
    </div>
...
.body :deep(hr) { border: none; border-top: 1px solid #333; margin: 12px 0; }
.body :deep(ul) { padding-left: 22px; margin: 6px 0; }
.body :deep(a) { color: #6cf; }
.body :deep(.empty) { color: #666; font-style: italic; }
</style>
