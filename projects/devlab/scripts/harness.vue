...
onMounted(async () => {
  try {
    node = await (await fetch(`/api/nodes/${SELF}`)).json()
    ctxText.value = node.ctx ?? ''
  } catch {
    statusText.value = 'ctx load failed — is the ocraft api service running?'
  }
})

const saveCtx = async () => {
  if (!node) {
    statusText.value = 'node not loaded'
    return
  }
  node.ctx = ctxText.value
  const res = await fetch(`/api/nodes/${SELF}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(node),
  })
  statusText.value = res.ok ? 'ctx saved' : `ctx save: HTTP ${res.status}`
}
...
<template>
  <div class="flex max-w-3xl flex-col gap-3">
    ...
    <textarea
      ...
      placeholder="ctx-sys-prompt — system prompt sent with every request (persisted on this node)"
    ></textarea>
    ...
    <div class="flex items-center gap-2">
      ...
      <button class="btn btn-sm" @click="saveCtx">save ctx</button>
