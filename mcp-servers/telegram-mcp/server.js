...
async function resolveEntity(ref) {
  ...
  if (/^-?\d+$/.test(s)) {
    try { return await client.getEntity(Number(s)) } catch { /* fall through */ }
  }
  ...
  throw new Error(`Could not resolve chat "${ref}". Use @username, numeric id, or exact title (see tg_list_chats).`)
}
...
const formatMessage = (m) => ({
  ...
  sender: m.sender ? (m.sender.username ? '@' + m.sender.username : (m.sender.title ?? m.sender.firstName ?? null)) : null,
...
server.registerTool(
  'tg_list_chats',
  {
    ...
    description: 'List your recent chats/dialogs (users, groups, channels) with their id, title, type, and unread count. Use the returned id or @username to address other tools.',
    inputSchema: { limit: z.number().int().min(1).max(500).default(50).describe('Max chats to return') },
  },
  async ({ limit }) => {
    try {
      const dialogs = await client.getDialogs({ limit })
      return ok(dialogs.map((d) => ({
        id: String(d.id),
        title: d.title,
...
server.registerTool(
  'tg_read_messages',
  {
    ...
    description: 'Fetch the most recent messages from one chat (channel, group, or private chat). Returns sender, text, date, and message id.',
    inputSchema: {
...
server.registerTool(
  'tg_get_chat_history',
  {
    ...
    description: 'Fetch older messages from a chat by paginating. Pass offsetId (the id of the oldest message you already have) to get the page before it.',
    inputSchema: {
      chat: z.string().describe('@username, numeric id, or exact title of the chat'),
      limit: z.number().int().min(1).max(200).default(50).describe('Messages per page'),
      offsetId: z.number().int().optional().describe('Return messages older than this message id (omit for newest)'),
    },
  },
...
server.registerTool(
  'tg_search',
  {
    ...
    description: 'Native MTProto full-text search. With "chat", searches inside that one chat; without it, searches globally across all your chats.',
    inputSchema: {
      query: z.string().min(1).describe('Text to search for'),
      chat: z.string().optional().describe('Optional: restrict to this @username/id/title. Omit for global search.'),
...
server.registerTool(
  'tg_send_message',
  {
    ...
    description: 'Send a text message to one chat (private chat, group, or channel you can post to). Optionally reply to an existing message. Returns the sent message id, date, and text.',
    inputSchema: {
...
server.registerTool(
  'tg_send_file',
  {
    ...
    description: 'Send a local image (or any file) to one chat, with an optional caption. Use a local filesystem path. Returns the sent message id, date, and caption.',
    inputSchema: {
      chat: z.string().describe('@username, numeric id, or exact title of the chat to send to'),
